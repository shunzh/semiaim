...
the lateral control cannot be as precise.  In fact, the human driver
cannot make a sharp turn while the adaptive cruise control is
...
For simple cruise control, the maneuverability profile is
written in Lisp syntax as follows:
{\small
\begin{verbatim}
...
responsible for ensuring that the trajectories encoded by the
...
request, it will conduct an internal simulation of the vehicle
...
Our hypothesis is that constraint-based requests with more restrictive
will perform better because SemiAIM reserves less tiles for them,

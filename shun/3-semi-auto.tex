...
vehicles often fails to obtain reservations when they are
...
to enter an intersection during red signal phases as well.
...
vehicles, which in turn subsume Type SA-Com vehicles because the human
driver can always opt to simply follow the traffic signals without
using cruise control if engaging the equipment is too distracting at
any given time.
...
we developed to simulate semi-autonomous vehicles at intersections.\footnote{Modified from the open-source AIM4 simulator: \url{http://www.cs.utexas.edu/~aim}}.
In this figure, the yellow vehicles are fully autonomous, the green
vehicles are Type SA-ACC, the blue vehicles are Type SA-CC, the white
vehicles are Type SA-Com, and the magenta vehicles are human-driven
vehicles.  Notice that although the traffic signals at the
intersection are red (see the red dots at the edges of the
intersection), fully autonomous vehicles can still enter the
intersection. In fact, all vehicles except the human-driven vehicles
